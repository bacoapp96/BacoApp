document.addEventListener("DOMContentLoaded", () => {
    console.log("Cervezas cargados");
    fetch("https://bacoapp.onrender.com/api/productos/categoria/Cervezas")
    .then(res => res.json())
...
const modal = document.getElementById("modal-cerveza");
const cerrar = document.querySelector(".cerrar");

async function mostrarDetallesCerveza(id) {
    try {
        const respuesta = await fetch(`https://bacoapp.onrender.com/api/productos/${id}`);
        const cerveza = await respuesta.json();

        console.log(cerveza);

        document.getElementById("id").textContent = cerveza.id;
        document.getElementById("nombre").textContent = cerveza.nombre;
        document.getElementById("descripcion").textContent = cerveza.descripcion;
        document.getElementById("precio").textContent = cerveza.precio;
        document.getElementById("stock").textContent = cerveza.stock;
        document.getElementById("categoria").textContent = cerveza.categoria;
        document.getElementById("marca").textContent = cerveza.marca;
        document.getElementById("ventas").textContent = cerveza.ventas;
        document.getElementById("fecha_creacion").textContent =
        new Date(cerveza.fecha_creacion).toLocaleString("es-CO");
...
async function cargarFiltros(){
    ...
    const response = await fetch (
         "https://bacoapp.onrender.com/api/productos/filtros/Cervezas"
    );
