document.addEventListener("DOMContentLoaded", () => {
    console.log("Jugos cargadas");
    fetch("https://bacoapp.onrender.com/api/productos/categoria/Jugos")
    .then(res => res.json())
...
const modal = document.getElementById("modal-jugos");
const cerrar = document.querySelector(".cerrar");

async function mostrarDetallesJugos(id) {
    try {
        const respuesta = await fetch(`https://bacoapp.onrender.com/api/productos/${id}`);
        const jugos = await respuesta.json();

        console.log(jugos);

        document.getElementById("id").textContent =jugos.id;
        document.getElementById("nombre").textContent =jugos.nombre;
        document.getElementById("descripcion").textContent =jugos.descripcion;
        document.getElementById("precio").textContent =jugos.precio;
        document.getElementById("stock").textContent =jugos.stock;
        document.getElementById("categoria").textContent =jugos.categoria;
        document.getElementById("marca").textContent =jugos.marca;
        document.getElementById("ventas").textContent =jugos.ventas;
        document.getElementById("fecha_creacion").textContent =
        new Date(jugos.fecha_creacion).toLocaleString("es-CO");
        ...
        console.error("Error al obtener jugos:", error);
    }
}
...
async function cargarFiltros(){
    ...
    const response = await fetch (
         "https://bacoapp.onrender.com/api/productos/filtros/Jugos"
    );
